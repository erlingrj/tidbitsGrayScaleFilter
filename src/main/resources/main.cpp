...
bool Run_GrayScale(WrapperRegDriver * platform, unsigned char *rgb_image, int rgb_size, unsigned char *grayscale_image) {
	GrayScale t(platform);
	...
	t.set_rgbAddr((AccelDblReg) accelBuf);
	t.set_grayAddr((AccelDblReg) accelBufRes);
	t.set_rgbSize(rgb_size_aligned);
	t.set_graySize(grayscale_size_aligned);
...
int main(int argc, char **argv)
{
  if(argc != 2) {
	...
	Run_GrayScale(platform, rgb_data, rgb_size, grayscale_data);
